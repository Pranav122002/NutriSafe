...
const SelectedStore = () => {
  ...
  const filterFoodItems = (items) => {
    return items.filter(
      (foodItem) =>
        foodItem.name.toLowerCase().includes(searchInput.toLowerCase()) // Step 2
    );
  };
      ...
      {selectedStore && (
        <div className="mt-20 w-4/6 ml-auto mr-10 text-left">
          ...
          <input
            ...
            placeholder="Search "
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)} // Step 1
            className="mt-4 w-5/6 p-2 border rounded"
          />
                  ...
                  {Object.entries(departments).map(([department, items]) => (
                    <div className="mt-4" key={department}>
                      ...
                      <ul className="flex w-5/6 flex-wrap">
                        {filterFoodItems(items).map((foodItem) => (
                          <li
                            className="ml-5 bg-white mt-4 border-3 p-3 rounded-lg w-60"
                            key={foodItem._id}
                          >
                            <img
